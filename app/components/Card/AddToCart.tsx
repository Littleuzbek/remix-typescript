...
export default function AddToCart({ product }: Props) {
  ...
  const addItem = (e: React.MouseEvent<HTMLButtonElement>): void => {
    ...
    const addingItem = {
      ...
      specs: product.specs,
      proType: product.proType || product.category,
      quantity: 1,
    };
  ...
  return (
    <button
      className="toCard W-[2.5rem] h-[2.2rem] p-[2px] m-[5px] rounded-[50%] cursor-pointer flex justify-center items-center duration-300 border-none bg-[white] hover:bg-[var(--second-color)]"
      onClick={(e) => {
        e.stopPropagation();
        addItem(e);
      }}
    >
      <svg
        ...
        className="w-full h-full hover:bg-[rgb(241, 240, 240)]"
      >
        <path
