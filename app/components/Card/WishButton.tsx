...
import { Product as ProductProp } from "~/utils";

type Props = {
  product: ProductProp;
};

export default function WishButton({ product }: Props) {
  const wishes = useSelector(
    (state: RootState) => state.cart.wishes as ProductProp[]
  );
  const inWish = wishes.find((item) => item.id === product.id);
  const dispatch = useDispatch();

  const addToWishes = (e: React.MouseEvent<HTMLButtonElement>): void => {
    e.preventDefault();
  ...
  return (
    <button
      onClick={(e) => addToWishes(e)}
      className="toWishes w-[2rem] h-[2rem] absolute z-[1] top-[.2rem] right-[.4rem] cursor-pointer border-none bg-[transparent]"
    >
      {inWish ? (
        <svg
          width="20"
        ...
        <svg
          ...
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="iconFull w-full h-full text-[blueviolet]"
        >
          <path
